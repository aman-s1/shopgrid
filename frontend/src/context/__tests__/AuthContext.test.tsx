...
const TestComponent = () => {
    ...
    return (
        <div>
            ...
            <button onClick={() => login({ id: '1', username: 'test', email: 'test@example.com', role: 'user' })}>
                Login
            </button>
...
describe('AuthContext', () => {
    ...
    it('shows loading initially and verifies session if token exists', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/auth/me'), expect.any(Object));
    });
    ...
    it('handles login action correctly', async () => {
        ...
        expect(screen.getByTestId('auth-status')).toHaveTextContent('Logged In');
        expect(screen.getByTestId('user-email')).toHaveTextContent('test@example.com');
        expect(JSON.parse(localStorage.getItem('user') || '{}')).toMatchObject({ email: 'test@example.com' });
    });
    ...
    it('handles logout action correctly', async () => {
        ...
        // User is logged out because the login method must be called to set state
        // Let's first log them in via the button
        act(() => {
            screen.getByText('Login').click();
        });
