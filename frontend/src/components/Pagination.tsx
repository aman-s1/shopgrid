          ...
          {getPageNumbers().map((page, index) =>
            typeof page === 'number' ? (
              <button
                ...
                className={`group relative flex h-11 w-11 cursor-pointer items-center justify-center rounded-2xl text-sm font-bold transition-all duration-500 ${
                  currentPage === page
                    ? 'bg-gradient-to-br from-blue-500 via-indigo-500 to-indigo-600 text-white shadow-xl ring-2 shadow-blue-500/30 ring-white/20'
              ...
              <span
                key={index}
                className="flex h-11 w-8 items-center justify-center font-bold text-gray-600"
              >
                {page}
              </span>
            )
