...
export function Navbar({ onOpenFilter, activeFilter, onSearch, initialSearch = '' }: NavbarProps) {
            ...
            <div className="layout-container h-full">
                <div className="flex h-full items-center justify-between">
                    <div className="flex items-center space-x-8">
                        ...
                        {/* Search Input */}
                        <div className="relative hidden max-w-md flex-1 items-center md:flex ml-10">
                            <div className="group relative w-full">
                                <MagnifyingGlassIcon className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-500 transition-colors group-focus-within:text-blue-400" />
                                <input
                                    type="text"
                                    placeholder="Search products..."
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                    className="w-full rounded-2xl border border-white/5 bg-white/5 py-2.5 pl-12 pr-10 text-sm font-medium text-white transition-all placeholder:text-gray-500 focus:border-blue-500/30 focus:bg-white/10 focus:outline-none focus:ring-4 focus:ring-blue-500/10"
                                />
                                {searchTerm && (
                                    <button
                                        onClick={() => setSearchTerm('')}
                                        className="absolute right-3 top-1/2 flex -translate-y-1/2 cursor-pointer items-center justify-center rounded-full p-1 text-gray-500 hover:bg-white/10 hover:text-white"
                                    >
